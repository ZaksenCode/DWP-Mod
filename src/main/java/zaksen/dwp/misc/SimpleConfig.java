...
public class SimpleConfig {
    ...
    private SimpleConfig( ConfigRequest request ) {
        ...
        if( !request.file.exists() ) {
            Dwp.LOG.info( identifier + " is missing, generating default one..." );

            try {
                createConfig();
            } catch (IOException e) {
                Dwp.LOG.error( identifier + " failed to generate!" );
                Dwp.LOG.trace( e + "" );
                broken = true;
            }
        }
        ...
        if( !broken ) {
            try {
                ...
                Dwp.LOG.error( identifier + " failed to load!" );
                Dwp.LOG.trace( e + "" );
                broken = true;
            }
        }
    ...
    public boolean delete() {
        Dwp.LOG.warn( "Config '" + request.filename + "' was removed from existence! Restart the game to regenerate it." );
